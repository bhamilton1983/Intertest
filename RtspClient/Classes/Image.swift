...
public func contrast(image: UIImage) -> RGBA {
    ...
    for y in 0..<rgba.height {
      ...
      let index = y * rgba.width + x
            ...
            pixel.red = UInt8(max(min(255, avgRed + 1 * redDelta), 0))
            pixel.green = UInt8(max(min(255, avgGreen + 1 * greenDelta), 0))
            pixel.blue = UInt8(max(min(255, avgBlue + 12 * blueDelta), 0))
...
public func play(image: UIImage) -> RGBA {
    let rgba = RGBA(image: image)!
    var totalRed = 0
    var totalGreen = 0
    var totalBlue = 0
    ...
    for y in 0..<rgba.height {
        for x in 0..<rgba.width {
            ...
            let greenDelta = Int(pixel.green) - avgGreen
            let blueDelta = Int(pixel.blue) - avgBlue
            pixel.red = UInt8(max(min(255, avgRed + 10 * redDelta), 0))
            pixel.green = UInt8(max(min(255, avgGreen + 10 * greenDelta), 0))
            pixel.blue = UInt8(max(min(255, avgBlue + 10 * blueDelta), 0))
            rgba.pixels[index] = pixel
        }
    }
...
public func playWith(image: UIImage) -> RGBA {
    let rgba = RGBA(image: image)!
    var totalRed = 0
    var totalGreen = 0
    var totalBlue = 0
    ...
    let avgGreen = totalGreen / pixelCount
    let avgBlue = totalBlue / pixelCount
    let avgAlpha = totalAlpha / pixelCount
    ...
    for y in 0..<rgba.height {
        for x in 0..<rgba.width {
            ...
            let greenDelta = Int(pixel.green) - avgGreen
            let blueDelta = Int(pixel.blue) - avgBlue
            let alphaDelta = Int(pixel.alpha)
            pixel.red = UInt8(max(min(255, avgRed + 1 * redDelta), 0))
            pixel.green = UInt8(max(min(255, avgGreen + 1 * greenDelta), 0))
            pixel.blue = UInt8(max(min(255, avgBlue + 1 * blueDelta), 0))
            pixel.alpha = UInt8(max(min(255, avgAlpha + 56 * alphaDelta), 0))
            rgba.pixels[index] = pixel
        }
    }
